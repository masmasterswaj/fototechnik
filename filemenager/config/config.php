...
# ************************************************************************************************************************************************************
	...
	if(!isset($_GET['nrArt']))
		{
		if(!isset($_SESSION['pathFileUpload']))
			$_SESSION['pathFileUpload'] = '/artykuly/00000/';
		}else{
			$_SESSION['pathFileUpload'] = '/artykuly/'.$_GET['nrArt'].'/';	
		}
	$pathminiaturaUpload = substr($_SESSION['pathFileUpload'],-6);
//------------------------------------------------------------------------------
...
$upload_dir = $_SESSION['pathFileUpload']; // path from base_url to base of upload folder (with start and final /)
$current_path = '..'.$_SESSION['pathFileUpload']; // relative path from filemanager folder to upload folder (with final /)
//thumbs folder can't put inside upload folder
$thumbs_base_path = '../miniatury/'.$pathminiaturaUpload; // relative path from filemanager folder to thumbs folder (with final /)
...
$image_resizing = TRUE;
$image_resizing_width  = 900;
$image_resizing_height = 0;

# Wielkosc obrazka docelowego dla artykulu D
$newD_w = 600; 
$newD_h = 600;  
# Wielkosc obrazka docelowego dla miniatur galerii S
$newS_w = 250; 
$newS_h = 250; 
# Wielkosc obrazka docelowego dla miniatur galerii SS
$newSS_w = 150; 
$newSS_h = 150; 
...
$fixed_path_from_filemanager            = array('..'.$_SESSION['pathFileUpload']); //fixed path of the image folder from the current position on upload folder
$fixed_image_creation_name_to_prepend   = array('',''); //name to prepend on filename
$fixed_image_creation_to_append         = array('_s',''); //name to appendon filename
$fixed_image_creation_width             = array(150,''); //width of image (you can leave empty if you set height)
$fixed_image_creation_height            = array(150,''); //height of image (you can leave empty if you set width)
/*
...
$relative_path_from_current_pos         = array('..'.$_SESSION['pathFileUpload']); //relative path of the image folder from the current position on upload folder
$relative_image_creation_name_to_prepend= array('',''); //name to prepend on filename
$relative_image_creation_name_to_append = array('_d',''); //name to append on filename
$relative_image_creation_width          = array(900,''); //width of image (you can leave empty if you set height)
$relative_image_creation_height         = array(900,''); //height of image (you can leave empty if you set width)
/*
  #             $option:     0 / exact = defined size;
  #                          1 / portrait = keep aspect set height;
  #                          2 / landscape = keep aspect set width;
